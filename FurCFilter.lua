...
function FurC.SetFilter(useDefaults, skipRefresh)
  ...
  showAllOnTextSearch = FurC.GetFilterAllOnText()
    and #searchString > 0
    and FURC_NONE == ddSource
    and FURC_NONE == dropdownChoiceVersion
    and FURC_NONE == dropdownChoiceCharacter
...
local function matchSourceDropdown()
  ...
  if recipeArray.origin == FURC_CRAFTING then
    if ddSource == FURC_CRAFTING then
      return true
    end
...
local function matchCraftingTypeFilter()
  if not recipeArray.origin == FURC_CRAFTING then
    return false
  end
